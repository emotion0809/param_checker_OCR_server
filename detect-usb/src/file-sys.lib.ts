...
export let fileSys = {
    ...
    readFileSync(pathToFileWithExt: string, encoding: 'utf8' | 'binary' = 'utf8') {
        let data = fs.readFileSync(pathToFileWithExt, encoding);
        return data;
    },
    readFileSyncWithDefaultStr(pathToFileWithExt: string, defaultStr = '', encoding: 'utf8' | 'binary' = 'utf8') {
    ...
    /** 為了相容node 4.9.1 */
    copyFileP(sourcePath: string, desitinationPath: string) {
        desitinationPath = getUniqueName(desitinationPath);
        this.makeIfNoDir(this.getDirFromFilePath(desitinationPath));
        return new Promise<void>((res, rej) => {
            copyFile(sourcePath, desitinationPath, (err) => {
                if (err) rej(err);
                else res();
            })
        })
        function copyFile(sourcePath: string, desitinationPath: string, cb = (err?: Error) => { }) {
            var cbCalled = false;
